...
function applyMiniPickerSelection(entry, mapId, gridPos) {
    ...
    // Apply Speed and Power if present in the entry (essential for MOPA)
    if (entry.speed !== undefined) layer.speed = entry.speed;
    if (entry.power !== undefined) layer.power = entry.power;
...
/**
 * Update the Test Grid Modal UI based on active device (MOPA vs UV)
 */
function updateTestGridUI() {
    const s = state.settings;
...
function saveSettings() {
    ...
    state.settings = {
        ...
        blackPower: parseFloat(document.getElementById('settingBlackPower').value),
        ...
        defocus: elements.settingDefocus ? normalizeDefocus(parseFloat(elements.settingDefocus.value)) : normalizeDefocus(state.settings.defocus),
    };
...
// Idea 8 — Smart range suggestions.
// Merge calibrated per-material/per-laser defaults (material-registry) over the
// static fallbacks so axis ranges and constants start from known-good values.
function getActiveSettingsKey() {
    const deviceId = resolveDeviceId(state.settings?.activeDevice || 'f2_ultra_uv');
    const laserTypeId = state.settings?.activeLaserType || null;
...
function getFlexSuggestions(isMopa) {
    ...
    let md = {};
    try { md = getMaterialDefaults(materialId, settingsKey) || {}; } catch { md = {}; }

    const out = JSON.parse(JSON.stringify(base));
    const apply = (param, defKey, rangeKey) => {
        if (typeof md[defKey] === 'number') out[param].def = md[defKey];
        if (Array.isArray(md[rangeKey]) && md[rangeKey].length === 2) {
            out[param].range = [md[rangeKey][0], md[rangeKey][1]];
        }
    };
    apply('frequency', 'frequency', 'freqRange');
    apply('power', 'power', 'powerRange');
...
function getCustomGridSettings() {
    ...
    if (isMopaLike) {
        ...
        const settings = {
            ...
            lpi: parseInt(document.getElementById('gridLpiFixed').value) || 5000
        };

        // Populate Ranges/Fixed based on mode
        if (fixedMode === 'frequency') {
            // Fixed Frequency, Vary Power & Speed
            settings.freq = parseInt(document.getElementById('gridFreqFixed').value) || 200;
            ...
            settings.speedMin = parseInt(document.getElementById('gridSpeedMin').value) || 200;
            settings.speedMax = parseInt(document.getElementById('gridSpeedMax').value) || 1200;

        } else if (fixedMode === 'power') {
            // Fixed Power, Vary Freq & Speed
            settings.power = parseInt(document.getElementById('gridPower').value) || 14;

            settings.freqMin = parseInt(document.getElementById('gridFreqMin').value) || 200;
            settings.freqMax = parseInt(document.getElementById('gridFreqMax').value) || 1200;
...
async function analyzeGridImage(img) {
    ...
    if (qrResult.found && !qrResult.error) {
        ...
        if (s.t === 'flex') {
            ...
            analyzerState.mopaLpc = s.l || 5000;
...
function saveColorMap() {
        ...
        for (let col = 0; col < numCols; col++) {
            ...
            if (!(col >= qrStartCol && row >= qrStartRow) && color && !color.isQR) {
                colorEntries.push({
                    color: color,
                    frequency: freq,
                    // Dynamic Mapping based on Device Type
                    lpi: analyzerState.isMopa ? (analyzerState.mopaLpc || 5000) : lpi,
                    speed: analyzerState.isMopa ? lpi : (SettingsStorage.getDefaults().speed || 425),
                    power: analyzerState.isMopa ? (analyzerState.mopaPower || 14) : (SettingsStorage.getDefaults().power || 70),
    ...
    const colorMapData = {
        ...
        gridImage: gridImage,
        deviceType: analyzerState.isMopa ? 'f2_ultra_mopa' : 'f2_ultra_uv'
    };
